...
Y = 0.5 * X + 2 + np.random.normal(0, 0.05, (400, )) # 假设我们真实模型为：Y=0.5X+2
# 绘制数据集plt.scatter(X, Y)
plt.show()

X_train, Y_train = X[:260], Y[:260]     # 把前160个数据放到训练集
X_test, Y_test = X[260:], Y[260:]       # 把后40个点放到测试集
...
# 测试训练好的模型
print('\nTesting ------------')
cost = model.evaluate(X_test, Y_test, batch_size=140)
